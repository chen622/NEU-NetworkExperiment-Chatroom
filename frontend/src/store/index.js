  ...
  mutations: {
    connect () {
      if (window.location.protocol === 'http:') {
        index.commit('initWebSocket', 'ws://' + 'localhost:8080' + '/websocket/chat')
      } else {
        index.commit('initWebSocket', 'wss://' + 'localhost:8080' + '/websocket/chat')
      }
    },
    initWebSocket (state, wsuri) {
      state.Chat.socket = new WebSocket(wsuri)
      state.Chat.socket.onopen = function () {
        index.commit('log', 'Info: WebSocket connection opened.')
      }
      state.Chat.socket.onclose = function () {
        index.commit('log', 'Info: WebSocket closed.')
      }
